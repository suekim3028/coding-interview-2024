...
#  
# 입력 예시
# 4 4         # 4 x 4 맵 생성1 1 0      # (1, 1)에 북쪽(0)을 바라보고 서 있는 캐릭터1 1 1 1   # 첫 줄은 모두 바다1 0 0 1   # 둘째 줄은 바다/육지/육지/바다1 1 0 1   # 셋째 줄은 바다/바다/육지/바다1 1 1 1   # 넷째 줄은 모두 바다
#  
# 출력 예시
# 3
...
def try1():
    ...
    res = 0;
    visited[row][col] = 1;
    while(0<=row<=n and 0<=col<=m and game_map[row][col] ==0):
        k = 0;
        for i in range(4):
            k+=1;
            next_d = 3 if d == 0 else (d-1);
            movement = move_arr[next_d];

            next_row = row+movement[0];            
            next_col = col+movement[1];

            if( (0<=next_row<=n ) and (0<=next_col<=m) and game_map[next_row][next_col]==0 and visited[next_row][next_col] == 0):
                d = next_d;
